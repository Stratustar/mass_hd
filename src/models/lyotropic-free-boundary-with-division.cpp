...
void LyotropicFreeBoundaryWithDivision::SetMasks()
{
  if(not division_count--)
  {
    ...
    for(unsigned k=0; k<DomainSize; ++k)
    {
      ...
      if(phi[k]>1e-3 and pressure < P_critical and random_real()<division_rate*phi[k] and !outside[k] and GetYPosition(k)<level)
      {
...
void LyotropicFreeBoundaryWithDivision::UpdateFields(bool first)
{
  ...
  for(unsigned k=0; k<DomainSize; ++k)
  {
    ...
    phi_tmp[k] += phi[k]*alpha*division_m - phi[k]*beta*death_m;
  }
