...
Bacterial::Bacterial(unsigned LX_, unsigned LY_, unsigned BC)
  : LyotropicWithDivision(LX_, LY_, BC)
{}

void Bacterial::Initialize()
{
  ...
  if(!isfinite(k_growth) || k_growth < 0)
    throw error_msg("k-growth must be finite and non-negative.");
  ...
  if(friction <= 0)
    throw error_msg("bacterial requires friction > 0.");
}

void Bacterial::Configure()
{
...
void Bacterial::ConfigureFromFrame()
{
...
void Bacterial::RelaxFreeEnergy()
{
...
void Bacterial::ResetHydrodynamics()
{
  double ftotal = 0.;
...
void Bacterial::ConfigurePhiNoise()
{
...
void Bacterial::ConfigurePhenotype()
{
  if(chi_config=="noise")
  {
    for(unsigned k=0; k<DomainSize; ++k)
...
void Bacterial::UpdatePhenotypeQuantities()
{
  for(unsigned k=0; k<DomainSize; ++k)
  {
...
double Bacterial::PhenotypePhiEpsilon() const
{
  return 1e-12;
}

bool Bacterial::HasPhenotypeMaterial(unsigned k) const
{
  return phi[k] > PhenotypePhiEpsilon();
}

bool Bacterial::HasMaterialFace(unsigned k, unsigned neighbour) const
{
  return HasPhenotypeMaterial(k) || HasPhenotypeMaterial(neighbour);
}

double Bacterial::MaskedPhiFaceIncrement(unsigned k, unsigned neighbour, double increment) const
{
  return HasMaterialFace(k, neighbour) ? increment : 0.;
}

double Bacterial::LocalMaterialValue(const ScalarField& fld, unsigned k, bool& found) const
{
  if(HasPhenotypeMaterial(k))
  {
    found = true;
...
double Bacterial::MaterialValue(const ScalarField& fld, unsigned preferred,
...
double Bacterial::TransportFaceValue(const ScalarField& fld, unsigned k,
...
double Bacterial::MemorySource(unsigned k, double m_eff) const
{
...
void Bacterial::ProjectDensity(ScalarField& fld)
{
  for(unsigned k=0; k<DomainSize; ++k)
  {
...
double Bacterial::ProliferationRate(unsigned k) const
{
  return alpha*(division_mask[k] ? 1. : 0.);
}

void Bacterial::UpdateQuantitiesAtNode(unsigned k)
{
  ...
  const double R_div = chi_eff*p*ProliferationRate(k);
  const double active_prefactor = zeta*p*(1-chi_eff) + k_growth*R_div;
...
void Bacterial::UpdateQuantities()
{
  // sum -> countphi
  double sum = 0;
...
void Bacterial::ComputeDryVelocityAtNode(unsigned k)
{
  const auto& d = get_neighbours(k);
...
void Bacterial::ComputeDryVelocity()
{
  #pragma omp parallel for num_threads(nthreads) if(nthreads)
  for(unsigned k=0; k<DomainSize; ++k)
...
void Bacterial::UpdateDryFieldsAtNode(unsigned k, bool first)
{
  const auto& d = get_neighbours(k);
  ...
  const double growth_rate = ProliferationRate(k) - beta*death_m;
  const double chi_eff = MaterialChi(k, k);
...
void Bacterial::UpdateFields(bool first)
{
  #pragma omp parallel for num_threads(nthreads) if(nthreads)
  for(unsigned k=0; k<DomainSize; ++k)
...
void Bacterial::BoundaryConditionsFields()
{
...
option_list Bacterial::GetOptions()
{
  ...
  options[0].add_options()
    ...
    ("k-growth", opt::value<double>(&k_growth),
     "extensile stress per proliferation source: sigma_growth = -k_growth*R_div*Q")
