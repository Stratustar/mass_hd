#ifndef MODELS_LYOTROPIC_WITH_DIVISION_STRESS_HPP_
#define MODELS_LYOTROPIC_WITH_DIVISION_STRESS_HPP_

#include "models/lyotropic.hpp"

class LyotropicWithDivisionStress : public Lyotropic
{
protected:
  /** Mask for growth
   *
  ...
  unsigned death_count, division_rate_count;

    // create a temporary list of centers
  std::vector<std::array<unsigned, 2>> centers;
  std::vector<double> division_count;

public:
  LyotropicWithDivisionStress(unsigned, unsigned, unsigned);
  ...
  void serialize_frame(Archive& ar)
  {
    ...
    ar & auto_name(centers);
  }
};

#endif//MODELS_LYOTROPIC_WITH_DIVISION_STRESS_HPP_
