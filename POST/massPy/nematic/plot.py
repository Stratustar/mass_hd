...
def director(frame, engine=plt, scale=False, avg=4):
    ...
    for i, j in product(np.arange(frame.LX, step=avg), np.arange(frame.LY, step=avg)):
        ...
        y.append(j + f*ny[i,j]/2.)
        y.append(None)
    engine.plot(x, y, color='k', linestyle='-', linewidth=.25)
...
def defects(frame, engine=plt, arrow_len=0, ms=3):
    ...
    for d in nematicPy.get_defects(frame.QQxx, frame.QQyx, frame.LX, frame.LY, arrow_len!=0):
        if d['charge'] == 0.5:
            ...
            engine.plot(d["pos"][0], d["pos"][1], 'b^', markersize=ms)
