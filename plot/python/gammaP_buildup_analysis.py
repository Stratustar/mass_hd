#!/usr/bin/env python3
"""Analyze GammaP dependence of phi and bulk-pressure buildup."""
...
ANALYSIS_ROOT = Path("analysis_outputs/08jun/gammaP_buildup_sweep_100")
PHI_CRITICAL = 1.2
...
def analyze_run(run_dir: Path) -> RunMetrics:
    gamma_p = parse_gamma(run_dir.parent.name)
    ...
    for path in frames:
        ...
        phi = field(data, "phi")
        # sigma_bulk is negative for compressive buildup in these runs.
...
def grouped(rows: list[RunMetrics], metric: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    gammas = np.asarray(sorted({row.gamma_p for row in rows}), dtype=float)
    means = []
    sems = []
    for gamma in gammas:
        vals = np.asarray([getattr(row, metric) for row in rows if row.gamma_p == gamma])
        means.append(float(vals.mean()))
        sems.append(float(vals.std(ddof=1) / np.sqrt(len(vals))) if len(vals) > 1 else 0.0)
    return gammas, np.asarray(means), np.asarray(sems)
...
def write_summary(rows: list[RunMetrics], path: Path) -> None:
    ...
    gammas = sorted({row.gamma_p for row in rows})
    with path.open("w") as handle:
        handle.write("GammaP sweep buildup summary\n")
        ...
        handle.write("\n")
        for gamma in gammas:
            handle.write(f"GammaP = {gamma:g}\n")
            sub = [row for row in rows if row.gamma_p == gamma]
...
def main() -> None:
    ...
    rows = [
        analyze_run(run_dir)
        for run_dir in sorted(OUTPUT_ROOT.glob("GammaP*/seed*"))
        if (run_dir / "frame10000.json").exists()
    ]
    if not rows:
        raise RuntimeError(f"No completed runs found under {OUTPUT_ROOT}")

    write_csv(rows, ANALYSIS_ROOT / "gammaP_buildup_metrics.csv")
    write_summary(rows, ANALYSIS_ROOT / "gammaP_buildup_summary.txt")
    plot_summary(rows, ANALYSIS_ROOT / "gammaP_buildup_summary.png")
